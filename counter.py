...
class CounterThread(QThread):
    ...
    def counter(self, permission, colorDict, frame, CountArea, mot_tracker, videoName):
        ...
        for i, item in enumerate(track_bbs_ids):
            ...
            # calc input time into Area
            if is_started[id] == False:
                start_time[id] = time.time()
                end_time[id] = time.time()
                print(str(id)+" start time-"+str(time.time()))
                is_started[id] = True
            else:
                is_ended[id] = True
            end_time[id] = time.time()
            print(str(id)+" end_time-"+str(time.time()))
            ...
            # if is_started[id] == True:
            #     # calc speed
            #     if end_time[id] == start_time[id]:
            #         speed[id] = -1
            #     else:
            #        speed[id] = 10 / (end_time[id] - start_time[id]) * 3600 / 1000

            # print(sum_speed[objectName2number(objectName)])
            ...
            #             boxColor,
            #             thickness=2)
            cv2.putText(frame, objectName, (x1 - 1, y1 - 3), cv2.FONT_HERSHEY_COMPLEX, 0.4,
                        boxColor,
                        thickness=2)
            ...
            if  self.history[id]["no_update_count"] > 5:
                ...
                if is_ended[id] == True:
                    ...
                    if end_time[id] == start_time[id]:
                        speed[id] = 0
                    speed[id] = 10 / (end_time[id] - start_time[id]) * 3600 / 1000
                    print(str(id)+" speed is "+str(speed[id]))
                    sum_speed[objectName2number(objectName)] = sum_speed[objectName2number(objectName)]+speed[id]
                    print(str(id)+" speed is "+str(speed[id]))
